 ...
 *
 * @return string
 * render list & small grid layout
 */

if ( ! class_exists( 'innovation_ruby_list_sgrid_layout' ) ) {
	class innovation_ruby_list_sgrid_layout {
		static function render( $ruby_options ) {
			...
			//render
			innovation_ruby_template_part::open_page_inner( 'page-layout-inner', $ruby_options['sidebar_position'], 'list-sgrid-layout', $ruby_options['big_first'] );
			...
			while ( have_posts() ) {
				...
				//render block
				if ( 0 != $ruby_counter % 5 ) {
					echo '<div class="post-list-sgrid-outer col-sm-6 col-xs-12">';
					get_template_part( 'templates/loop', 'small_grid' );
					echo '</div><!--#small grid outer-->';
					if ( 0 == $ruby_divider_counter % 2 ) {
						innovation_ruby_template_part::render_divider( 'post-list-sgrid-outer' );
						$ruby_divider_counter = 0;
					}
					$ruby_divider_counter ++;
				} else {
					echo '<div class="post-list-sgrid-outer col-xs-12">';
