...
class bk_social_counter extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ((isset($bk_soundcloud_user)&&(strlen($bk_soundcloud_user) != 0)) && (isset($bk_soundcloud_api)&&(strlen($bk_soundcloud_api) != 0))){
		  	$transient__name = 'bk_social_json_transient_soundcloud';
            ...
            if ( $cache === false || $cache == '' ) {
                ...
                //if(time() > get_option('bk_soundcloud_cache_time')) {
                    ...
                    if ($soundcloud_count >= 0 ) {
                        ...
                        set_transient( $transient__name, $soundcloud_count, $hours );
    				}
                //} 
            }       
        }
		...
		if (isset($bk_instagram_api)&&(strlen($bk_instagram_api) != 0)){
            $transient__name = 'bk_social_json_transient_instagram';
            $cache           = get_transient( $transient__name );
            if ( $cache === false || $cache == '' ) {
    			$instagram_count = 0;
                $instagram_username = '';
                ...
                if ($instagram_count >= 0 ) {
                    ...
                    set_transient( $transient__name, $instagram_count, $hours );
    			}        
            }
        }
		//facebook
		if (isset($bk_facebook_username)&&(strlen($bk_facebook_username) != 0)){
			$transient__name = 'bk_social_json_transient_facebook';
            $cache           = get_transient( $transient__name );
            if ( $cache === false || $cache == '' ) {
                $fan_count = bk_core::bk_count_social($bk_facebook_username, 'facebook');			
				update_option('bk_facebook_followers', $fan_count);
				update_option('bk_facebook_link', 'https://www.facebook.com/'.$bk_facebook_username);
                set_transient( $transient__name, $fan_count, $hours );
            }
		}
		//dribbble
		if (isset($bk_dribbble_username)&&(strlen($bk_dribbble_username) != 0)){
            ...
            $cache           = get_transient( $transient__name );
            if ( $cache === false || $cache == '' ) {
    			$followers_count = bk_core::bk_count_social($bk_twitter_id, 'dribbble');
    			if ( ! empty( $followers_count ) ) {
    				update_option('bk_dribbble_followers', $followers_count );
                    set_transient( $transient__name, $followers_count, $hours );
    			}  
            }                  
		}
        
        if(isset($bk_youtube_username)&&(strlen($bk_youtube_username) != 0)){
            $transient__name = 'bk_social_json_transient_youtube';
            $cache           = get_transient( $transient__name );
            if ( $cache === false || $cache == '' ) {
                ...
                //if(time() > get_option('bk_youtube_cache_time')) {               
                    ...
                    if (isset($subscriberCount) && ($subscriberCount > 0) ){
                        ...
                        set_transient( $transient__name, $subscriberCount, $hours );
                    }
                //}
            }
        } 
        if(isset($bk_pinterest_id)&&(strlen($bk_pinterest_id) != 0)){
			$transient__name = 'bk_social_json_transient_pinterest';
            $cache           = get_transient( $transient__name );
            if ( $cache === false || $cache == '' ) {
                ...
                if ( $json_data != null ) {
                    ...
                    set_transient( $transient__name, $followerCount, $hours );
                }
			}
        }        
		?>
